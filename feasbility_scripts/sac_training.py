...
def create_env(env_path, worker_id=3, time_scale=1.0):
    channel = EngineConfigurationChannel()
    unity_env = UnityEnvironment(env_path, side_channels=[channel], worker_id=worker_id, no_graphics=False)
    channel.set_configuration_parameters(time_scale=time_scale)
    return UnityToGymWrapper(unity_env)

def get_save_path(base_path, model_name):
...
if __name__ == '__main__':
    ...
    model = SAC("MlpPolicy", env_scene1, verbose=2, tensorboard_log=tensorboard_log_path_scene1, policy_kwargs=policy_kwargs)
    model.learn(total_timesteps=1200000, reset_num_timesteps=True, tb_log_name="train_scene1", callback=[checkpoint_callback_scene1])
    ...
    env_scene2 = create_env(scene2_env, worker_id=4, time_scale=1.0)
    ...
    model = SAC.load(final_model_path_scene1, env=env_scene2, tensorboard_log=tensorboard_log_path_scene2, policy_kwargs=policy_kwargs, verbose=2)
    
    reward_logging_callback_scene2 = RewardLoggingCallback(log_interval=2000, log_file=f'reward_log_{model_name}.txt')

    model.learn(total_timesteps=1000000, reset_num_timesteps=False, tb_log_name="finetune_scene2", callback=[checkpoint_callback_scene2, reward_logging_callback_scene2])
    ...
    env_scene3 = create_env(scene3_env, worker_id=5, time_scale=1.0)
    ...
    model = SAC.load(final_model_path_scene2, env=env_scene3, tensorboard_log=tensorboard_log_path_scene3, policy_kwargs=policy_kwargs, verbose=2)

    reward_logging_callback_scene3 = RewardLoggingCallback(log_interval=2000, log_file=f'reward_log_{model_name}.txt')

    model.learn(total_timesteps=1000000, reset_num_timesteps=False, tb_log_name="finetune_scene3", callback=[checkpoint_callback_scene3, reward_logging_callback_scene3])
